...
class NetworkTest : public ::testing::Test {
 protected:
  void SetUp() override { filename_ = "../proto/model.prototxt"; }
  ...
  std::vector<std::pair<std::vector<Dtype>, Dtype>> generate_test_data() {
    // y = w0 + w1*x
    int low = -20;
    int high = 20;
    std::set<int> x;
    ...
    while (x.size() < 10) {
      ...
      x.insert(i);
      auto p = std::make_pair<std::vector<Dtype>, Dtype>({Dtype(i)}, func(i));
      res.emplace_back(p);
    }
...
TYPED_TEST(NetworkTest, fprop2) {
  ...
  for (int i = 0; i < 6000; i++) {
    ...
    ax_plus_by<TypeParam>(w.total_, -learning_rate, &dw[0], 1, &w[0]);

    ax_plus_by<TypeParam>(b.total_, -learning_rate, &db[0], 1, &b[0]);
